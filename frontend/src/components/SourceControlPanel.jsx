...
function ChangeRow({ change, group, selected, onDiff, onPreview, onRun, onHistory, onConfirm }) {
  ...
  const discardGit = async () => {
    ...
    const reason = window.prompt("Reason for discarding this Git change"); if (!reason) return;
    const approval = await api.issueApproval("git.discard", change.project, change.path, reason);
    await onRun(() => api.gitDiscard(change.project, change.path, group === "staged", group === "untracked", reason, approval.approval_token), `Discarded ${change.path}`);
  };
  ...
  const forceProposal = async () => {
    if (!(await onConfirm(`Force Apply and Stage Bob proposal for "${change.path}"? This can overwrite local edits and bypass a failed review.`))) return;
    const reason = window.prompt("Reason for force applying and Git-staging this proposal"); if (!reason) return;
    const approval = await api.issueApproval("proposal.override_apply", change.project, change.proposal_id, reason);
    await onRun(() => api.proposalOverrideApply(change.project, change.proposal_id, change.path, reason, approval.approval_token), `Force applied and staged ${change.path}`);
  };
  return (
    <div className={`scm-change-row vscode-scm-row ${selected ? "selected" : ""}`}>
      ...
      <div className="scm-row-actions">
        ...
        {proposal && (conflict || change.review_status === "FAIL") && <button title="Force Apply and Stage" onClick={forceProposal}><ShieldAlert size={14} /></button>}
...
export default function SourceControlPanel() {
  ...
  const discardAll = async () => { if (!(await confirmDialog("Discard all Git changes, untracked files, and Bob proposals? This cannot be undone."))) return; const reason = await promptDialog("Reason for discarding all source-control changes"); if (!reason) return; const approval = await api.issueApproval("git.discard_all", currentProject, "all", reason); await run(async () => { await api.gitDiscardAll(currentProject, true, reason, approval.approval_token); await api.proposalDiscardAll(currentProject); }, "Discarded all source-control changes"); };
  const forceAllProposals = async () => { if (!(await confirmDialog("Force apply and Git-stage all Bob proposals?"))) return; const reason = await promptDialog("Reason for forcing all proposals"); if (!reason) return; const approval = await api.issueApproval("worktree.apply_all", currentProject, "all", reason); await run(() => api.worktreeApplyAll(currentProject, true, reason, approval.approval_token), "Force applied and staged Bob proposals"); };
